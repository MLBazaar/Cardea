    ...
    def assert_type(self):
        ...
        if self.system is not None:
            for value in self.system:
                if value != None and value.lower() not in ['phone', 'fax', 'email', 'pager', 'url', 'sms', 'other']:
                    raise ValueError('"{}" does not match possible values: {}'.format(value, 'phone, fax, email, pager, url, sms, other'))

        if self.use is not None:
            for value in self.use:
                if value != None and value.lower() not in ['home', 'work', 'temp', 'old', 'mobile']:
                    raise ValueError('"{}" does not match possible values: {}'.format(value, 'home, work, temp, old, mobile'))
