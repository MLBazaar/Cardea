    ...
    def assert_type(self):
        ...
        if self.mode is not None:
            for value in self.mode:
                if value != None and value.lower() not in ['source', 'queried', 'target', 'produced']:
                    raise ValueError('"{}" does not match possible values: {}'.format(value, 'source, queried, target, produced'))
    ...
    def assert_type(self):
        ...
        if self.typeMode is not None:
            for value in self.typeMode:
                if value != None and value.lower() not in ['none', 'types', 'type-and-types']:
                    raise ValueError('"{}" does not match possible values: {}'.format(value, 'none, types, type-and-types'))
...
class StructureMap_Input(fhirbase):
    ...
    def assert_type(self):
        ...
        if self.mode is not None:
            for value in self.mode:
                if value != None and value.lower() not in ['source', 'target']:
                    raise ValueError('"{}" does not match possible values: {}'.format(value, 'source, target'))
    ...
    def assert_type(self):
        ...
        if self.listMode is not None:
            for value in self.listMode:
                if value != None and value.lower() not in ['first', 'not_first', 'last', 'not_last', 'only_one']:
                    raise ValueError('"{}" does not match possible values: {}'.format(value, 'first, not_first, last, not_last, only_one'))
...
class StructureMap_Target(fhirbase):
    ...
    def __init__(self, dict_values=None):
        ...
        # type = string
        # type = string
        # possible values = create, copy, truncate, escape, cast, append, translate, reference, dateOp, uuid, pointer, evaluate, cc, c, qty, id, cp
        # possible values = create, copy, truncate, escape, cast, append, translate, reference, dateOp, uuid, pointer, evaluate, cc, c, qty, id, cp
    ...
    def assert_type(self):
        ...
        if self.contextType is not None:
            for value in self.contextType:
                if value != None and value.lower() not in ['type', 'variable']:
                    raise ValueError('"{}" does not match possible values: {}'.format(value, 'type, variable'))

        if self.listMode is not None:
            for value in self.listMode:
                if value != None and value.lower() not in ['first', 'share', 'last', 'collate']:
                    raise ValueError('"{}" does not match possible values: {}'.format(value, 'first, share, last, collate'))

        if self.transform is not None:
            for value in self.transform:
                if value != None and value.lower() not in ['create', 'copy', 'truncate', 'escape', 'cast', 'append', 'translate', 'reference', 'dateop', 'uuid', 'pointer', 'evaluate', 'cc', 'c', 'qty', 'id', 'cp']:
                    raise ValueError('"{}" does not match possible values: {}'.format(value, 'create, copy, truncate, escape, cast, append, translate, reference, dateop, uuid, pointer, evaluate, cc, c, qty, id, cp'))
