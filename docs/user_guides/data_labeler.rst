...
Contiuning from :ref:`data_assembler`, we can now use ``select_problem`` with the desired prediction problem to generate ``label_times``.
...
.. note:: 
    you can use ``cardea.list_problems()`` to view available prediction problems.
